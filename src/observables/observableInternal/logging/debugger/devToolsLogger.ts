...
import { BugIndicatingError } from '../../../deps';

function onUnexpectedError(error: Error): void {
	console.error('Unexpected error in observable logger', error);
}
...
export class DevToolsLogger implements IObservableLogger {
	...
	private _getDeclarationId(type: IObsDeclaration['type']): number {
		...
		while (true) {
			...
			let result = getFirstStackFrameOutsideOf(stack, /[/\\]observableInternal[/\\]|\.observe|[/\\].*[Uu]til(s)?\./);
